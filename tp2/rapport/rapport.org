...
\begin{center}
    ...
    {\huge \bfseries TP Aide à la décision - JS\\[2cm]}
...
** Enoncé du problème
   ...
   On cherche à résoudre un problème de job shop.
   Un job shop correspond à une séquence de passage dans des machines pour un
   ensemble de pièces. Chaque pièces passent dans chaque machine mais dans des
   ordres différents et en y passant des temps différents. Notre but est de
   minimiser le temps nécessaires pour que toutes les pièces ai finit leur
   processus.
   
** Structure et Choix d'implémentation

   Il nous faut stocker plusieurs informations tout du long de notre étude du
   job shop. Pour cela nous avons séparer nos données en 2 structures distinctes:
   ...
   + Une structure =solution_t= avec:
     ...
     - Le temps pour une pièce et une opération données.
     - L'opération *père* pour une pièce et une opération données. (c'est à dire
     qu'elle est l'opération effectué juste avant de réaliser celle-ci).
     ...
     - Le hachage de cette solution. (utile pour le GRASP).
  
* Implémentation

** Lecture Dans un fichier

   Nous avons déjà un choix de représentation des données du job shop si on suit
   la formalisation de brunel. Cette représentation est sous la forme:

   + Une première ligne donnant le nombre de machine et de pièces
...
L'évaluation nécessite d'avoir au préalable générer un *vecteur de Bierwith* avec
la fonction dédiée. Le but de l'évaluation est de trouver les dates au plus tôt
pour un vecteur de Bierwith donné, et donc de mettre à jours les tableaux =dates=
...
l'évaluation, on calcule aussi le *hasch* de la solution, se *hash* sera utilisé par
la suite.
...
donc une boucle qui parcours le vecteur. A chaque tour de boucle, on met à
...
On commence par mettre à jours la date (et le père) suivant le lien
...
n'est mis à jours que pour les pièces de rang supérieur à 1 puisqu'une pièce
de rang 1 n'a pas de père *TODO: trouver un moyen de l'expliquer mieux*.
...
est modifié si $pieceSurMachine + cout(pieceSurMachine) > cout(piece)$.
...
À la fin on met à jour la pièce sur la machine et on met à jours le coût de la
solution si besoin.
...
#+begin_src C
  ...
  if (rangPiece == instance.nbMachines) {
    if (solution.dates[pieceActuelle][rangPiece]
      ...
      // mise à jours du cout et du père de '*'
    }
   }
  }
#+end_src
...
*** Raisonnement
   ...
   niveau locale, c'est à dire qu'on va prendre en argument une solution
   actuelle, et on va chercher toutes les modification locales possibles
   ...
   La logique de cette algorithme est donc de parcourir notre chemin critique en
   ...
   solution en permutant cette fois-ci les deux opérations à l'origine de cette
   arc. Si la solution ainsi trouvé est meilleur ont repart du noeud de fin
   (étoile) et on recommence. Sinon (arc non disjonctif ou pire coût) on
   continue notre parcours du chemin critique.

*** Implémentation
    
   Pour pouvoir permuter nos 2 noeuds dans le vecteur, on doit d'abord
   connaître leur position dans le vecteur de bierwith, C'est le but de notre
   fonction =recherchePosition= qui va nous prendre la pièce et le range de
   notre noeud courant et précédent et va nous renvoyer leur position dans le
   vecteur de bierwith. Pour cela on parcours le vecteur en sens inverse et dès
   qu'on voit notre pièce, on décrémente son rang. Dès qu'on atteint 0, on à
   trouvé notre index. Une optimisation apporté est de séparer le while on deux,
   en effet une fois qu'on à trouvé notre noeud courant, plus besoin de le
   chercher on peut se concentrer seulement sur le précédent.
...
Une fois cette fonction écrite, nos parties se résume à:
...
traité, on test si le coût est plus faible que celui de la meilleure solution
trouvée jusqu'ici, on met à jour la meilleur solution.
